...
( function( $ ) {
			...
			this.each( function() {
				...
				if ( editor && !instanceLock ) {
					...
					editor.on( 'instanceReady', function( evt ) {
						console.log("ckeditor.on.instanceReady");
						var editor = evt.editor;

						setTimeout( function() {
							console.log("ckeditor.on.instanceReady timeout");

							// Delay bit more if editor is still not ready.
							if ( !editor.element ) {
								setTimeout( function(){
									console.log("editor.once.instanceReady timeout callee");
									arguments.callee();
								}, 100 );
								return;
							}
							...
							editor.on( 'dataReady', function() {
								console.log("editor.on-dataReady");
							...
							if ( editor.config.autoUpdateElementJquery && $element.is( 'textarea' ) && $( element.form ).length ) {
								var onSubmit = function() {
									console.log("editor.onSubmit");
									$element.ckeditor( function() {
										console.log("editor.onSubmit.jqanonym");
								...
								$element.bind( 'destroy.ckeditor', function() {
									console.log("Unbind when editor destroyed.");
							...
							editor.on( 'destroy', function() {
								console.log("ckeditor.on-destroy");
					...
					editor.once( 'instanceReady', function( evt ) {
						console.log("editor.once.instanceReady");
						setTimeout( function() {
							console.log("editor.once.instanceReady timeout");
							// Delay bit more if editor is still not ready.
							if ( !editor.element ) {
								setTimeout( function(){
									console.log("editor.once.instanceReady callee");
									arguments.callee();
								}, 100 );
								return;
							}
	...
	if ( CKEDITOR.config.jqueryOverrideVal ) {
		console.log("CKEDITOR.config.jqueryOverrideVal");
		$.fn.val = CKEDITOR.tools.override( $.fn.val, function( oldValMethod ) {
			console.log("CKEDITOR.tools.override");
			return function( value ) {
				// Setter, i.e. .val( "some data" );
				console.log("Setter, i.e. .val( \"some data\" );");
				if ( arguments.length ) {
					var _this = this,
						promises = [], //use promise to handle setData callback

						result = this.each( function() {
							console.log("if-arguments.this.each");
							var $elem = $( this ),
							...
							// Call default .val function for rest of elements
								...
								console.log("Call default .val function for rest of elements");
					...
					// Create one promise which will be resolved when all of promises will be done.
						...
						$.when.apply( this, promises ).done( function() {
							console.log("dfd.resolveWith( _this )");
